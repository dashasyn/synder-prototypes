...
### Filtering Options Prototype (2026-04-30; v2 functional, v3 multiselect, v4 Apply-gated, v5 six variants — all 2026-08-04; v7 variant 2026-08-07; v8 V6/V7 refinements 2026-08-11; v9 Variant 8 + v10 V7 rework 2026-08-20)
- **Status:** ✅ v10 live and verified — eight variants, all Apply-gated, on Synder's real 8-status taxonomy. 297 jsdom + 190 Chromium checks across nine suites, all in `scripts/` (no longer `/tmp/`).
...
- **Variants:**
  ...
  7. **Button + Side sheet** — One "Filters" button opens a right-hand sheet holding **every** filter incl. status (all 8); applied filters show as read-only chips under the toolbar (removable, not editable); badge counts selected values; toolbar search commits on Enter. **No segments row and no count line** (v10)
...
- **v10 (2026-08-20) — Variant 7 rework.** Ignat: *"make date selection to look the same as other filters · add status filter with all statuses · remove Showing 2 of 26 transactions · 3 filters applied"*. I asked which of two readings he meant on the first two; he didn't answer, so I picked defaults and flagged them.
  - **Status is a field in the sheet again, with all 8 statuses — and the segments row is GONE.** He didn't say to remove it, but keeping both puts two controls on one dimension, which is exactly FLT-2 (pick *Failed* in the sheet, click the *Ready to sync* segment, they disagree). This reverts the Aug-11 change where I *added* segments to V7 and pushed status out of the sheet. Net effect: **V7 is now the "everything in one sheet" shape and V8 carries the group-tabs idea** — a cleaner pair than both variants having tabs. One-line revert if he wanted both.
  - **Single-select panels render as radio rows** — new `radioOptions` ctx flag on `optionListHtml`. Date/Amount/Customer were plain rows with a checkmark sitting next to Platform/Type's checkbox lists; now every panel in the sheet reads as the same *kind* of control. Behaviour unchanged (one choice, commits on pick). **Scoped to V7's sheet only** — variants 1–6 and 8 keep the checkmark rows, so the blast radius is what he asked about and nothing else.
  - **Count line removed.** The chips row already names every applied filter and the badge carries the count, so *"Showing N of M · K filters applied"* was a third readout of the same state. V8 keeps its version because it says something the chips can't — *"Showing 1 of 7 in Attention required"*, the fraction the tab count deliberately refuses to show.
  - **Knock-ons:** status gets a chip like every other dimension · badge counts status values · sheet Reset clears status too (nothing lives outside the sheet now) · `Clear all` has no segment to snap back · `renderSheetbtnSegments()` deleted (`renderSegmentsInto` stays, V6 and V8 still use it).
  - **Suites moved out of `/tmp/` into `scripts/`** so they stop rotting between sessions: `verify-filters.cjs` (148), `verify-v7.cjs` (41, new), `verify-v8.cjs` (108), `browser-v7.cjs` (32), `browser-v7b.cjs` (39, new), `browser-v8.cjs` (71), `browser-v6v7.cjs` (25), `browser-check.cjs` (21), `v8-footer.cjs` (2). **Stale V7-segment assertions in three existing suites were rewritten against the new design, not deleted.**
  - **Open:** the dropdown panels are 210px min-width inside a full-width sheet field, so a panel is narrower than its trigger. Equally true of every field, so it isn't a date-vs-others inconsistency — but it's the obvious next polish if he wants the sheet to read as one system.
- **Design decisions:**
