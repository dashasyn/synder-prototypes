// v9 — clean comb branching: horizontal bar + vertical drops, no dangling stubs
const fs = require('fs');
...
function renderNodes(node) {
    ...
    for (const { type, dx } of nOff) {
        ...
        svg += `<rect x="${nx + dx}" y="${ny - L.tickH / 2}" width="${L.tickW}" height="${L.tickH}" rx="1" fill="${tc}"/>`;
    }

    // Metro icon
    if (node.isMetro && !isRoot) {
        const leftDx = nOff[0].dx;
        svg += `<circle cx="${nx + leftDx - 14}" cy="${ny}" r="6.5" fill="#E74C3C"/>`;
        svg += `<text x="${nx + leftDx - 14}" y="${ny + 3.5}" text-anchor="middle" font-size="9" font-weight="700" fill="#fff">М</text>`;
    }

    // Stop name — wrapped
    if (!isRoot) {
        const rightDx = nOff[nOff.length - 1].dx;
        const lx = nx + rightDx + L.tickW + 5;
...
fs.writeFileSync('preview-v9.svg', full);
